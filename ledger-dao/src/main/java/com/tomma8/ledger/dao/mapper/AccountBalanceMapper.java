...
public interface AccountBalanceMapper {
    ...
    @Insert("INSERT INTO account_balance (account_id, balance_type, position, currency, amount, account_seq, last_journal_id) " +
            "VALUES (#{accountId}, #{balanceType}, #{position}, #{currency}, #{amount}, #{accountSeq}, #{lastJournalId}) " +
    ...
    @Select("SELECT account_id, balance_type, position, currency, amount, frozen_amount, locked_amount, account_seq, last_journal_id " +
            "FROM account_balance WHERE account_id = #{accountId}")
    List<Map<String, Object>> findByAccountId(@Param("accountId") String accountId);

    @Select("SELECT account_id, balance_type, position, currency, amount, frozen_amount, locked_amount, account_seq, last_journal_id " +
            "FROM account_balance WHERE account_id = #{accountId} AND balance_type = #{balanceType} AND position = #{position} AND currency = #{currency}")
